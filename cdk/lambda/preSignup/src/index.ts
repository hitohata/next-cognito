...
export const handler: PreSignUpTriggerHandler = async (event: PreSignUpTriggerEvent, context, callback) => {
    ...
    try {
        ...
        const providerNameWithCapital = providerName.charAt(0).toUpperCase() + providerName.slice(1);
...
const getUserDate = async ({userPoolId, emailAddress}: {userPoolId: string, emailAddress: string}): Promise<UserType[]> => {
